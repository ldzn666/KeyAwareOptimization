...
//                List<Path> allPath = rt.readPath(pathStr);
                ...
                for(Path p : allPath){
                    ...
                    c: for (int k = 0; k < p.size() - 1; k++) {
                        ...
                        //4.计算当前密钥池容量
                        int curKeyPoolNum = topoLink.virtualLink.get(curr,next).getKeyPool() + keyPoolSupply - keyPoolConsume;
                        arr[k] = curKeyPoolNum;
...
//                SupplymentAlgorithm.KeyDynamicSupplyment(topoLink,event);
    ...
    public static void main(String[] args) throws Exception {
        AlgorithmByKeySupplyVVV oabk = new AlgorithmByKeySupplyVVV();
