...
import static al.KeySupply.KeyAwareSupply;
...
public class KSDRA {
    ...
    public void startKSDRA(double q,double p){
        ...
        while((event = eventTrace.poll()) != null){
            ...
            cutIndex.clear();
            KeyAwareSupply(topoLink,event,q,p);
    ...
    public static void main(String[] args) {
        System.out.println("补充算法优化前：");
        NonKSDRA nonKSDRA = new NonKSDRA();
        nonKSDRA.startNonKSDRA();

        for(double i = 0; i <= 1; i += 0.2){
            System.out.println("补充算法优化后：" + "A=" + i +"..."+ "B=" + (1-i));
            KSDRA ksdra = new KSDRA();
            ksdra.startKSDRA(i,1-i);
        }
